...
<div class="show_if_seller" style="<?php echo esc_attr( $role_style ); ?>">
    ...
    <p class="form-row form-group form-row-wide">
        ...
        <small><?php echo esc_url( home_url() . 'seller-registration-form.php/' . dokan_get_option( 'custom_store_url', 'dokan_general', 'store' ) ); ?>/<strong id="url-alart"></strong></small>
    </p>
    ...
    <!-- Custom field for file in pdf that is the visura camerale-->
    <p class="form-row form-group form-row-wide">
        <label for="shop-visura-camerale"><?php esc_html_e( 'Visura camerale', 'dokan-lite' ); ?><span class="required">*</span><span style="font-size: 10pt">(PDF < 5MB)</span></label>
        <input type="file" class="input-text form-control" name="visura_camerale" id="shop-visura-camerale" value="<?php echo ! empty( $data['visura_camerale'] ) ? esc_attr( $data['visura_camerale'] ) : ''; ?>" required="required"
...
<script>
    document.getElementById('shop-visura-camerale').addEventListener('change', function (e) {
        var file = this.files[0];

        // Check if the file size exceeds 5MB
        if (file.size > 5 * 1024 * 1024) {
            alert('Errore: Il file caricato supera i 5MB.');
