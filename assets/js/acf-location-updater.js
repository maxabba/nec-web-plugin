    ...
    const AcfLocationUpdater = {
        ...
        checkExistingState: function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'acf_updater_test'
                },
                success: (response) => {
                    console.log('AJAX Test Result:', response);
                    if (response.success) {
                        // Connection working, now check for existing state
                        $.ajax({
                            url: acf_updater.ajax_url,
                            type: 'POST',
                            data: {
                            ...
                            error: (xhr, status, error) => {
                                console.error('Error checking state:', xhr.responseText);
                                this.handleError('Errore nel controllo dello stato esistente');
                            }
                        });
                    }
                },
                error: (xhr, status, error) => {
                    ...
                    this.handleError('Errore di connessione AJAX: ' + error);
                }
            });
        },
        ...
        processBatch: function(phase, offset) {
            const phaseNames = {
                1: 'Inserimento Città Mancanti',
                2: 'Inserimento Provincie Mancanti'
            };
